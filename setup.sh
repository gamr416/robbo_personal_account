...
usage() {
  cat <<'EOF'
setup.sh — развернуть локальный стек ЛК (БД + backend + frontend)

Опции:
  --pull           Обновить субмодули frontend/backend до origin/main
  --skip-build     Не пересобирать образы app/web (только docker compose up -d)
  -h, --help       Эта справка
...
main() {
  ...
  log "Инициализация git-субмодулей (frontend, backend)..."
  if [[ "$PULL_SUBMODULES" == true ]]; then
    git -C "$ROOT" submodule update --init --recursive
    git -C "$ROOT" submodule update --remote --merge frontend backend
  else
    git -C "$ROOT" submodule update --init --recursive
  fi

  for dir in "$PROJECTS_DB_DIR" "$BACKEND_DIR" "$FRONTEND_DIR"; do
  ...
  log "[5/5] Frontend React (порт 3030)..."
  if [[ "$SKIP_BUILD" == false ]]; then
    (cd "$FRONTEND_DIR" && docker compose build web)
