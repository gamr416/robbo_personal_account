...
- **Scratch / PROJECT DB (PostgreSQL, единый источник правды по ученическим Scratch-проектам в ЛК)**:
  ...
  - Таблицы: **`scratch_projects`** — карточка проекта и ссылочный ключ редактора: `id` (= `storage_project_id`/`projectRef`), `owner_user_id` (= `sub`/id пользователя в JWT), **`title`, `instruction`, `note`, `is_public`** (ранее в `project_page_dbs`), **`scratch_vm_json`** (перенос прежнего JSON из `project_dbs.json`, REST `/project/`), версии **`version_counter`**, **`current_version_id`**, `created_at`, `updated_at`, `deleted_at`; **`scratch_project_legacy_map`** — соответствие старых `project_dbs.id` / `project_page_dbs.id` новому UUID (обратная совместимость URL после hard switch).
  - **`scratch_project_versions`**, **`scratch_project_audit_events`** — как раньше (`.sb3` в BYTEA и аудит).
  - **Backend ЛК**: читает/пишет проекты только в PROJECT DB; DSN задаётся `projectsPostgres.postgresDsn` / env **`PROJECTS_POSTGRES_DSN`**.
  - **Перенос из старой ЛК БД** (до выката): `robbo_projects_db/scripts/backfill_lk_projects.py`.
- **Хранение проектов**:
...
- В UI ЛК пункт «Мои курсы» заменён на «LMS»; маршрут `/mycourses` не показывает старый список курсов — выполняется переход в LMS (`https://lms2.robbo.world`, либо OIDC authorize при заполненной конфигурации).
...
_Текущий контур (май 2026):_ карточка «Мои проекты» — **Projects DB** (`robbo_projects_db`); Robbo-метаданные ЛК — **`robbo_portal_*`** в той же PostgreSQL; LMS — **MySQL read + REST**, без DDL в `openedx`. SSO: Open edX = IdP, ЛК = OIDC client ([ADR_LK_TWO_DATABASES.md](ADR_LK_TWO_DATABASES.md), [LMS_HANDOFF.md](LMS_HANDOFF.md)).
